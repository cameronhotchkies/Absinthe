...
//   $Id: Sql7ErrorPlugin.cs,v 1.7 2006/07/15 01:48:58 nummish Exp $
//
...
public class Sql7ErrorPlugin : IErrorPlugin
{
	public string PluginDisplayTargetName
	{
		get
		{
			return "SQL Server 7 Error Plugin";
		}
	}
	...
	public string[] KnownSupportedVersions
	{
		get
		{
			return new string[]  
			{
				@"Microsoft SQL Server  7.00 - 7.00.\d\d\d \(Intel X86\) \n\t(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (\d| )\d 20\d\d \d\d:\d\d:\d\d \n\tCopyright \(c\) 1988-199\d Microsoft Corporation\n\t(Standard|Developer|Enterprise) Edition on Windows NT 5.\d \(Build \d\d\d\d: (Service Pack \d)?\)"
			};
		}
	}
